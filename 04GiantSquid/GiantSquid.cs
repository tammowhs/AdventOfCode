	...
	public class GiantSquid : ICodePuzzle
	{
		public int EvaluatePartOne(string input)
        {
            ...
            foreach (var drawnNumber in drawnNumbers)
            {
                ...
                {
                    var field = bingoBlock.Find(f => f.Value == drawnNumber);

                    if (field is not null)
                    {
                        field.Marked = true;

                        if (CheckBingoBlockForWin(bingoBlock))
                        {
                            var sumOfUnmarked = bingoBlock
                                .Where(f => !f.Marked)
                                .Sum(f => f.Value);
        ...
        private void DeserializeInput(string input, out List<int> drawnNumbers, out List<List<Field>> bingoFields)
        {
            ...
            bingoFields = inputBlocks.Select((iBString, blockIndex) => DeserializeBingoBlock(iBString, blockIndex)).ToList();
        }
		...
		public List<Field> DeserializeBingoBlock(string blockInput, int blockIndex)
		{
